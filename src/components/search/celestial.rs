...
pub fn SearchBar() -> impl IntoView {
    ...
    let celestials = create_rw_signal(Vec::new());
    ...
    let on_input = move |_| {
        ...
        let matched = matching_left(&celestials, value.clone());

        input.set(value.clone());

        // filters items based on input
        filtered_vec(&filtered_items, &celestials, &input);
    ...
    view! {
        <main class="flex flex-col py-2">
            ...
            <section name="Container">
                <div id="supported-celestial-container" class="hidden">
                    <div class="
                        ...
                        ">
                        <section id="supported-celestials" class="flex flex-col overflow-y-auto h-min font-bold overflow-scroll">
                            <Celestial_Items vector=filtered_items input=input/>
                        </section>
                    </div>
                </div>
            </section>
...
pub fn Celestial_Items(
    ...
    let append_input = move |ev: MouseEvent| {
        ...
        update_dom_el("supported-celestial-container", "hidden");
    };
    ...
    view! {
        <For
            ...
            let:data
        >
            <span on:click=append_input class="p-2 cursor-pointer dark:bg-slate-900 bg-slate-200">{data}</span>
        </For>
    }
}
